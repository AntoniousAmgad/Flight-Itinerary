...
// methods below this point are added new
  ...
  bool isPath(int fromCity, int toCity, stack<flightRec> & flightstack);
  ...
  void displayItinerary(stack<flightRec>& flightStack, string originCity, string destinationCity);

private:
  int size;              // number of cities
  vector<string> cities; // list of cities
  map<string,list<flightRec>> map;  // flight map
